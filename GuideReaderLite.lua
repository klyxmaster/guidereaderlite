...
f.text:SetNonSpaceWrap(true)   -- helps long tokens; safe to keep
...
function GRL:IsStepDone(i)
    ...
    if t.qid or a == "H" or a == "h" or a == "R" or a == "E" or (a == "C" and t.qids and #t.qids > 0) then
		...
		if a == "E" then
			-- Examine/Read/Use → auto-advance when the spawned quest (|AQ|) is accepted,
			-- or shortly after opening a readable (scroll/book/letter).
			local ok = false
			local aq = tonumber(t.AQ)
			if aq and self._recentAccepted and _recent(self._recentAccepted[aq], 15) then
				ok = true
			elseif self._recentReadAt and (GetTime() - self._recentReadAt) <= 10 then
				ok = true
			end
			result = ok
		end
        ...
        if a == "T" then
			...
			if t.qid then
				if self.turnedinquests and self.turnedinquests[t.qid] then
					ok = true
				elseif self._recentTurnedIn and _recent(self._recentTurnedIn[t.qid], 10) then
					ok = true
				end
			end
			result = ok
...
function GRL:UpdateStatusFrame()
    ...
    local render = (top or "") .. (note or "")
	render = render:gsub("\\n", "\n"):gsub("<br>", "\n")
	self.status.text:SetText(render)
...
GRL:RegisterEvent("ITEM_TEXT_BEGIN", function(self)
    self._recentReadAt = GetTime()
...
GRL:RegisterEvent("QUEST_ACCEPTED", function(self, ...)
    -- Args are (questLogIndex, questID) on Classic; some cores flip them.
    local a, b = ...
    local qid = tonumber(b) or tonumber(a)
    if not qid then return end

    -- Make sure caches exist
    self._recentAccepted = self._recentAccepted or {}

    -- Stamp and nudge the stepper
    self._recentAccepted[qid] = GetTime()

    -- Advance immediately on the real accept event
    if self.AutoAdvance then self:AutoAdvance(true) end
end)
